...
from ..teams_service import send_task_teams_notifications
...
def _notify_category(task: Task):
    ...
    try:
        send_task_notification(
            to_email=normalized_recipients,
            subject=f'[TASK-{task.id}] {task.title}',
            tasks=[task],
            template_type='new_task',
        )
    except Exception as exc:
        current_app.logger.warning('Notifica email categoria fallita task_id=%s: %s', task.id, exc)

    try:
        teams_result = send_task_teams_notifications(
            task,
            normalized_recipients,
            action='new_task',
        )
        if teams_result.get('failed'):
            current_app.logger.warning(
                'Notifica Teams parzialmente fallita task_id=%s: %s',
                task.id,
                teams_result,
            )
    except Exception as exc:
        current_app.logger.warning('Notifica Teams categoria fallita task_id=%s: %s', task.id, exc)
